...
import { ReactVNODE, Primitive, ReactElement } from './model'
...
export function render(
  vnode: ReactVNODE | Primitive,
  parent: ReactElement | null = null
) {
  ...
  if (isStrOrNumber(vnode)) {
    ...
    console.warn(`ERROR ReactVNODE:`)
    console.log(vnode)
  }
}

const __ReactKeyedElmsPool: Map<string, ReactElement> = new Map()

/**
 * Diff old dom and fresh vnode, then patch
 * @param dom old DOM
 * @param vnode fresh virtual DOM
...
export function patch(
  ...
  if (typeof vnode === 'object' && isFunction(vnode.type)) {
    ...
    if (instance && instance.renderFunc === vnode.type) {
      const shouldComponentUpdate = !isEqual(instance.props, vnode.children, ['children'])
    ...
    const unkeyedElmsPool: Map<string, ReactElement> = new Map()

    const oldChildrenNodes = Array.from(dom.childNodes) as ReactElement[]
    ...
    // 新增和更改 DOM
    flatten(vnode.children).forEach((newChildVnode: ReactVNODE, index) => {
    ...
    unkeyedElmsPool.forEach(restElm => {
      const instance = restElm.__react__
      if (instance) {
...
function patchAttributes(dom: ReactElement, { props: newProps }) {
  const oldAttrs = {}
 ...
 */
function setAttribute(elm: ReactElement, attr: string, value: any) {
